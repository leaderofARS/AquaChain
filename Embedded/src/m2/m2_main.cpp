...
// Inline configuration (moved from config.h for convenience)
// WARNING: Do not commit real secrets (Wi-Fi password, tokens) to a public repo.
static const char* WIFI_SSID    = "ARS8608";
static const char* WIFI_PASS    = "WPA-1ARS";
static const char* BACKEND_HOST = "192.168.137.1";  // Backend on hotspot IP
...
int readSoilRaw() {
  // On some boards you may set attenuation, e.g.: analogSetPinAttenuation(SENSOR_PIN, ADC_11db);
...
void applyDecision(float avgMoisture) {
  ...
  // Manual override from control API
...
String canonicalizeSnapshot(const JsonDocument& doc) {
  // Serialize with sorted keys to keep canonical form
  String out;
...
void truncateQueueBy(size_t flushed) {
  // Rewrite remaining lines to a temp file then replace
...
void pollControlIfNeeded(unsigned long& lastPollMs) {
  ...
  if (http.begin(client, url)) {
    ...
    if (code >= 200 && code < 300) {
      ...
      // Expecting JSON array of commands or an object
...
void loop() {
  ...
  if (nowMs() - lastSampleMs >= SAMPLE_INTERVAL_MS) {
    ...
    // Post at interval; otherwise, queue-only
    if (nowMs() - lastPostMs >= POST_INTERVAL_MS) {
      ...
      // Even between posts, queue the snapshot if WiFi down, so we don't lose data
      if (WiFi.status() != WL_CONNECTED) appendQueueLine(body);
    }
  }

  // Periodic queue flush and control poll
